function [processedSig,templateArray_cell,template] = templateSubtract_dictionary_iterative(raw_sig,varargin)
%USAGE:
...
for i=1:2:(length(varargin)-1)
    switch lower(varargin{i})
        ...
        case 'start_inds'
            start_inds = varargin{i+1};
        case 'end_inds'
            end_inds = varargin{i+1};
    end
end
...
start_inds = cellfun(@(x) x+10,start_inds,'un',0);
end_inds = cellfun(@(x) x-20,end_inds,'un',0);
...
for chan = goodVec
    ...
    %end
      ...
      %  distanceDBscan = 1e-4;
    distanceDBscan = 7e-5;
    [c,ptsC,centres] = dbscan(templateArray,distanceDBscan,1,'euclidean');
