...
addParameter(p,'threshVoltageCut',95,@isnumeric);
addParameter(p,'threshDiffCut',95,@isnumeric);
...
for ind = 1:size(template,2)
    ...
    threshSig = pctl(absZSig,threshVoltageCut); % started with 95, then tried 90
    threshDiff = pctl(absZDiffSig,threshDiffCut); % started with 95, then tried 90 to better address early part 
