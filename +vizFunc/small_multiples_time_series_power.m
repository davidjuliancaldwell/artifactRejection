function small_multiples_time_series_power(signal,t,varargin)
    ...
    switch modePlot
        case 'avg'
            ...
            elseif ismember(idx,type2)
                plot(1e3*t,signal(:,idx),'Color',CT(2,:),'LineWidth',2)
                title([num2str(idx)],'Color',CT(2,:))
            else
                plot(1e3*t,signal(:,idx),'Color',CT(1,:),'LineWidth',2)
        ...
        case 'ind'
            if ismember(idx,type1)
                ...
                title([num2str(idx)],'Color',CT(3,:))
            elseif ismember(idx,type2)
                plot(1e3*t,squeeze(signal(:,idx)),'Color',CT(2,:),'LineWidth',2)
                title([num2str(idx)],'Color',CT(2,:))
            else
                plot(1e3*t,squeeze(signal(:,idx)),'Color',CT(1,:),'LineWidth',2)
                title([num2str(idx)],'color',CT(1,:))
            end
        ...
        case 'confInt'
            if ismember(idx,type1)
                ...
                title([num2str(idx)],'Color',CT(3,:))
            elseif ismember(idx,type2)
                vizFunc.plot_error(1e3*t',squeeze(signal(:,idx,:)),'CI',CT(2,:));
                title([num2str(idx)],'Color',CT(2,:))
            else
                vizFunc.plot_error(1e3*t'',squeeze(signal(:,idx,:)),'CI',CT(1,:));
                title([num2str(idx)],'color',CT(1,:))
            end
    end
...
obj.Position = [-100,-25];
obj.hTextX_Pos = [0,-1]; %move only the LABEL position
