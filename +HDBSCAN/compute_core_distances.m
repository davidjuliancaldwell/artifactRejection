function [dCore,D,kdtree] = compute_core_distances( X,k ,metric)
    ...
    %if (m >= 10) || (n < 100)
        kdtree = nan;
        if n > 20e3
            [~,dCore,D] = HDBSCAN.compute_nearest_neighbors( X,X,k-1 ); % slow but memory conservative
        else
            D = HDBSCAN.compute_pairwise_dist( X, [], metric );
            dCore = sort( D,1 );
            dCore = dCore(k,:);
        end
    %else
